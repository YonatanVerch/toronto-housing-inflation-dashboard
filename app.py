...
def load_regions():
    lower_shp = Path(__file__).parent / "data/Municipal_Boundary_-_Lower_and_Single_Tier.shp"
    upper_shp = Path(__file__).parent / "data/Municipal_Boundary_-_Upper_Tier_and_District.shp"

    lower_gdf = gpd.read_file(lower_shp, encoding="utf-8")
    upper_gdf = gpd.read_file(upper_shp, encoding="utf-8")

    # Compute centroids
    lower_gdf["LATITUDE"] = lower_gdf.geometry.centroid.y
    lower_gdf["LONGITUDE"] = lower_gdf.geometry.centroid.x
    upper_gdf["LATITUDE"] = upper_gdf.geometry.centroid.y
    upper_gdf["LONGITUDE"] = upper_gdf.geometry.centroid.x

    # Name columns
    lower_gdf = lower_gdf[["MUNICIPA_1", "LATITUDE", "LONGITUDE"]].rename(columns={"MUNICIPA_1": "NAME"})
    upper_gdf = upper_gdf[["OFFICIAL_M", "LATITUDE", "LONGITUDE"]].rename(columns={"OFFICIAL_M": "NAME"})

    # Normalize names
    lower_gdf["NAME_norm"] = lower_gdf["NAME"].astype(str).str.lower().str.replace(r"^(city of |town of |township of )", "", regex=True).str.strip()
    upper_gdf["NAME_norm"] = upper_gdf["NAME"].astype(str).str.lower().str.replace(r"^(city of |town of |township of )", "", regex=True).str.strip()

    return pd.concat([lower_gdf, upper_gdf], ignore_index=True)
...
def server(input, output, session):
    ...
    regions_df = load_regions()  # returns GeoDataFrame with NAME, LATITUDE, LONGITUDE
    ...
    # Merge with shapefile centroids
    df_map = regions_df.merge(
        housing_by_region,
        on="NAME",
        how="inner"  # Only include regions with housing data
    )
    ...
    def map_full():
        ...
        # Get the selected year from sidebar
        selected_year = int(input.selected_year())
        housing_year = housing_df[housing_df["Year"] == selected_year]
        ...
        # Fill missing values with 0 or None (so table formatting always works)
        avg_year = avg_year.fillna(0)
        ...
        for _, row in df_map.iterrows():
            ...
            loc_data = avg_year[avg_year["Location"] == name]

            if not loc_data.empty:
                loc_data = loc_data.iloc[0]  # there is now exactly 1 row per location
                table_html = "<table>"
                ...
                table_html = f"<i>No data available for {selected_year}</i>"  # still adds marker
